...
namespace Enigma
{
    ...
    void Model::processAnimation2() {
		for (unsigned int i = 0; i < m_Scene->mNumAnimations; ++i) {
            ...
            m_animations.back().ticksPerSecond =
                m_Scene->mAnimations[i]->mTicksPerSecond == 0 ? 30.f : m_Scene->mAnimations[i]->mTicksPerSecond;
            m_animations.back().duration = m_Scene->mAnimations[i]->mDuration;
            ...
            for (unsigned int j = 0; j < m_Scene->mAnimations[i]->mNumChannels; ++j) {
                ...
                auto dstChannel = const_cast<NodeAnim*>(findNodeAnim(m_animations.back(),node->name));
				if(!dstChannel){
					m_animations.back().channel.emplace_back();
					dstChannel = &m_animations.back().channel.back();
