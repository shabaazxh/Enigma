...
layout(set = 1, binding = 1) uniform sampler2D metallic[40];
...
void main() {
   ...
   gPosition = vec4(WorldPosition.xyz, 1.0);
   gNormal = normalize(vec4(WorldNormal, 1.0));
   albedo = vec4(vec3(texture(textures[push.textureIndex], uv).xyz), 1.0);
}
