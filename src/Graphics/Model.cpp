...
namespace Enigma
{
	...
	void Model::processMesh(aiMesh* mesh, const aiScene* scene) {
		...
		tempMesh.boneData = boneData;  // 存储骨骼数据
	...
	void Model::updateAnimation(const aiScene* scene, float deltaTime) {
		...
		// Get the number of frames and time information for the animation
		for (unsigned int i = 0; i < scene->mNumAnimations; i++) {
			aiAnimation* anim = scene->mAnimations[i];
			...
			updateNodeMatrices(scene->mRootNode, aiMatrix4x4(), animationTime, anim);
		}
	}

	void Model::updateNodeMatrices(const aiNode* node, const aiMatrix4x4& parentTransform, float animationTime, const aiAnimation* anim) {
		std::string nodeName(node->mName.data);
		aiMatrix4x4 nodeTransform = node->mTransformation;

		//Finds the animation node where the current node is in the animation
		const aiNodeAnim* nodeAnim = findNodeAnim(anim, nodeName);

		if (nodeAnim) {
			// Compute the interpolation transformation of the node
			aiMatrix4x4 scalingMat = interpolateScale(animationTime, nodeAnim);
			aiMatrix4x4 rotationMat = interpolateRotation(animationTime, nodeAnim);
			aiMatrix4x4 translationMat = interpolateTranslation(animationTime, nodeAnim);

			// Combine the transforms of the nodes
			nodeTransform = translationMat * rotationMat * scalingMat;
		}

		// Calculates the global transformation for the node
		aiMatrix4x4 globalTransform = parentTransform * nodeTransform;

		// If the node corresponds to a bone, update its final transform
		...
		// The function is executed recursively for all child nodes
		for (uint32_t i = 0; i < node->mNumChildren; i++) {
			updateNodeMatrices(node->mChildren[i], globalTransform, animationTime, anim);
		}
	}
