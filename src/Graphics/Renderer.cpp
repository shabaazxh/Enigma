...
namespace Enigma
{
	Renderer::Renderer(const VulkanContext& context, VulkanWindow& window, Camera* camera, World* world) : context{ context }, window{ window }, camera{ camera }, m_World{ world } {
		...
		m_pipeline = CreateGraphicsPipeline("../resources/Shaders/vertex.vert.spv", "../resources/Shaders/fragment.frag.spv", VK_FALSE, VK_TRUE, VK_TRUE, {Enigma::sceneDescriptorLayout, Enigma::descriptorLayoutModel}, m_pipelinePipelineLayout, VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, context, window);
		m_aabbPipeline = CreateGraphicsPipeline("../resources/Shaders/vertex.vert.spv", "../resources/Shaders/line.frag.spv", VK_FALSE, VK_TRUE, VK_TRUE, { Enigma::sceneDescriptorLayout, Enigma::descriptorLayoutModel }, m_pipelinePipelineLayout, VK_PRIMITIVE_TOPOLOGY_LINE_STRIP, context, window);
		m_animPipeline = CreateGraphicsPipeline("../resources/Shaders/vertexAnim.vert.spv", "../resources/Shaders/fragment.frag.spv", VK_FALSE, VK_TRUE, VK_TRUE, { Enigma::sceneDescriptorLayout, Enigma::descriptorLayoutModel,Enigma::boneTransformDescriptorLayout}, m_animPipelinePipelineLayout, VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, context, window);
		...
		// Rendering ( Record commands for submission )
		{
			...
			for (const auto& model : m_World->Meshes)
			{
                if(model->animations2.empty()){
				    ...
				    vkCmdBindPipeline(m_renderCommandBuffers[Enigma::currentFrame], VK_PIPELINE_BIND_POINT_GRAPHICS, m_animPipeline.handle);
				    model->Draw2(m_renderCommandBuffers[Enigma::currentFrame], m_animPipelinePipelineLayout.handle);
                    //draw aabb
				    // vkCmdBindPipeline(m_renderCommandBuffers[Enigma::currentFrame], VK_PIPELINE_BIND_POINT_GRAPHICS, m_aabbPipeline.handle);
				    // model->DrawAABB(m_renderCommandBuffers[Enigma::currentFrame], m_pipelinePipelineLayout.handle);
                }
			}
