...
namespace Enigma
{
	GBuffer::GBuffer(const VulkanContext& context, const VulkanWindow& window) : context{context}
	{
		...
		g_BufferOutputTargets.normals = Enigma::CreateImageTexture2D(
			context,
			m_width,
			m_height,
		...
		g_BufferOutputTargets.depth = Enigma::CreateImageTexture2D(
			context,
			m_width,
			m_height,
		...
		CreateFramebuffer(context.device, m_width, m_height);
	...
	{
		std::vector<VkImageView> attachments = { g_BufferOutputTargets.normals.imageView, g_BufferOutputTargets.depth.imageView };
