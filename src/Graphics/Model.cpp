...
namespace Enigma
{
	...
	void Model::loadBones(aiMesh* mesh, std::vector<Vertex>& boneData) {
		...
		for (uint32_t i = 0; i < mesh->mNumBones; i++) {
			...
			// Check if the bone already exists in the mapping, if not, add it
			...
			// Assign bone index to the mapping table
			boneMapping[boneName] = boneIndex;
			// Set the offset matrix from the mesh bone data
			boneInfo[boneIndex].offsetMatrix = aiMat4x4toMat4(mesh->mBones[i]->mOffsetMatrix);
			// Assign weights to vertices affected by the current bone
			for (uint32_t j = 0; j < mesh->mBones[i]->mNumWeights; j++) {
				uint32_t vertexID = mesh->mBones[i]->mWeights[j].mVertexId;
				float weight = mesh->mBones[i]->mWeights[j].mWeight;
				// Update the bone data for the vertex
	...
	*/
	...
	void Model::readNodeHierarchy(float animationTime, Node* node, const glm::mat4& parentTransform, const Animation& anim) {
		// Initialize node transformation from the node's transformation matrix
		glm::mat4 nodeTransform = node->transformation;

		// Find the animation data for this node by name
		const NodeAnim* nodeAnim = findNodeAnim(anim, node->name);
		if (nodeAnim) {
			// Calculate interpolated scaling matrix for the current animation time
			glm::mat4 scalingMat = interpolateScale(animationTime, nodeAnim);
			// Calculate interpolated rotation matrix for the current animation time
			glm::mat4 rotationMat = interpolateRotation(animationTime, nodeAnim);
			// Calculate interpolated translation matrix for the current animation time
			glm::mat4 translationMat = interpolateTranslation(animationTime, nodeAnim);

			// Combine the transformations: translation, then rotation, then scaling
			nodeTransform = translationMat * rotationMat * scalingMat;
		}

		// Combine parent transformation with current node transformation to get the global transformation
		glm::mat4 globalTransform = parentTransform * nodeTransform;

		// If this node corresponds to a bone, update its final transformation matrix
