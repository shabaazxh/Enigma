...
namespace Enigma
{
	...
	*/
	...
	void Model::readNodeHierarchy(float animationTime, Node* node, const glm::mat4& parentTransform, const Animation& anim) {
		glm::mat4 nodeTransform = node->globalMatrix;
		...
		for (auto child : node->children) {
			readNodeHierarchy(animationTime, child.get(), globalTransform, anim);
		}
	}
    ...
    void Model::LoadModelAssimp(const std::string& filepath) {
        ...
        for (unsigned int i = 0; i < scene->mNumMeshes; i++) processMesh2(scene->mMeshes[i]);

        // process node
        int tempIndex = 0;
        processNode2(scene->mRootNode, &rootNode, tempIndex);
		loadBones2();	
		processAnimation2();
    ...
    void Model::updateAnimation2(float deltaTime, int index){
        ...
        for (auto &e : animation.channel) {
            e.node->translation = interpolateTranslation2(animationTime,&e);
            e.node->rotation = interpolateRotation2(animationTime,&e);
            e.node->scale = interpolateScale2(animationTime,&e);
        }
        rootNode.Update();
        updateBoneTransforms2();
    }
    ...
    void Model::processNode2(aiNode* node, Node* dstNode, int& index) {
        aiVector3D scaling;
        ...
        for (unsigned int i = 0; i < node->mNumChildren; i++) {
            ...
            processNode2(node->mChildren[i], dstNode->children.back().get(), index);
        }
    }
    void Model::processMesh2(aiMesh* mesh) {
		Mesh tempMesh;
    ...
    void Model::processAnimation2() {
		for (unsigned int i = 0; i < m_Scene->mNumAnimations; ++i) {
            animations.emplace_back();
            animations.back().ticksPerSecond =
                m_Scene->mAnimations[i]->mTicksPerSecond == 0 ? 30.f : m_Scene->mAnimations[i]->mTicksPerSecond;
            animations.back().duration = m_Scene->mAnimations[i]->mDuration;
            ...
            for (unsigned int j = 0; j < m_Scene->mAnimations[i]->mNumChannels; ++j) {
                ...
                auto dstChannel = const_cast<NodeAnim*>(findNodeAnim(animations.back(),node->name));
				if(!dstChannel){
					animations.back().channel.emplace_back();
					dstChannel = &animations.back().channel.back();
    ...
    void Model::updateBoneTransforms2() { 
        updateBoneTransforms2Helper(&rootNode); 
    ...
    void Model::updateBoneTransforms2Helper(Node* node) {
        boneTransforms[node->index] = globalInverseTransform * node->globalMatrix * node->boneOffsetMatrix;
        for (auto&& e : node->children) updateBoneTransforms2Helper(e.get());
    }
}
